...
const App = () => {
  ...
  useEffect(() => {
    const fetchFixedExpenses = async () => {
      try {
        const res = await fetch(
          "https://kakeibo1-production.up.railway.app/api/fixed_expenses"
        );
        const data = await res.json();
      ...
      {selectedDate && (
        <ExpenseListByDate
          ...
          onDelete={async (id) => {
            ...
            const url = isFixed
              ? `https://kakeibo1-production.up.railway.app/api/fixed_expenses/${id}`
              : `https://kakeibo1-production.up.railway.app/api/expenses/${id}`;

...
const API_BASE_URL = "https://kakeibo1-production.up.railway.app"; // 🌐 ← 本番サーバーURL
      ...
      {selectedDate && (
        <ExpenseListByDate
          ...
          onDelete={async (id) => {
            const isFixed = fixedExpenses.some((f) => f.id === id);
            const url = isFixed
              ? `${API_BASE_URL}/api/fixed_expenses/${id}`
            ...
            if (res.ok) {
              if (isFixed) {
                const refreshed = await fetch(
                  `${API_BASE_URL}/api/fixed_expenses`
                );
                setFixedExpenses(await refreshed.json());
              } else {
                const refreshed = await fetch(`${API_BASE_URL}/api/expenses`);
                setExpenses(await refreshed.json());
              }
              setSelectedDate(null);
            }
